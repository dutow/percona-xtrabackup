  ...
  if (vault_curl->read_key(static_cast<const Vault_key &>(*key),
    ...
    logger->log(MY_ERROR_LEVEL, ("Could not read key from Vault." +
                                 get_errors_from_response(json_response))
                                    .c_str());
    fprintf(stderr, "Retrive vault key 1: %s\n", json_response.c_str());
    return true;
  }
  fprintf(stderr, "Retrive vault key 2: %s\n", json_response.c_str());
  return false;
}
...
bool Vault_io::write_key(const Vault_key &key) {
  ...
  if (vault_curl->write_key(key, &json_response) ||
    ...
    fprintf(stderr, "Wrote vault key 1: %s\n", json_response.c_str());
    return true;
  }
  fprintf(stderr, "Wrote vault key 2: %s\n", json_response.c_str());
  return false;
}
...
bool Vault_io::delete_key(const Vault_key &key) {
  ...
  if (vault_curl->delete_key(key, &json_response) ||
    ...
    return true;
    fprintf(stderr, "Delete vault key 1: %s\n", json_response.c_str());
  }
  fprintf(stderr, "Delete vault key 2: %s\n", json_response.c_str());
  return false;
}
